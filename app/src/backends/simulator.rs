...
pub fn setup_simulator_and_input(tx: Sender<InputEvent>) -> Backend<SimulatorDisplay<Rgb565>> {
    ...
    let backend_config: EmbeddedBackendConfig<SimulatorDisplay<Rgb565>, _> =
        EmbeddedBackendConfig {
            flush_callback: Box::new(move |display| {
                ...
                handle_pending_press_timeout(&mut pending_press, &tx_clone);
                process_simulator_events(&mut simulator_window, &mut pending_press, &tx_clone);
            }),
            ..Default::default()
        };

...
fetch_latest_release_json() {
    ...
    content=$(curl -s -w "%{http_code}" -o /tmp/amaru_curl_out "$url")
    http_code="$content"
    
    if [ "$http_code" -eq 200 ]; then
        cat /tmp/amaru_curl_out
        rm -f /tmp/amaru_curl_out
        return 0
...
stage_binary() {
    ...
    if [ -z "$extracted_bin" ]; then
        abort "Extracted file for ${binary_name} missing."
    fi
...
fn write_script(path: &str, content: &str) -> io::Result<()> {
    println!("Migration [2025_12]: Writing to {}", path);
    let p = Path::new(path);
    fs::write(p, content)?;
...
fn patch_amaru_service() -> anyhow::Result<()> {
    ...
    println!("amaru.service patched and reloaded.");
    Ok(())
}
